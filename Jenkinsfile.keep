    ...
    stages {
        ...
        stage('Nexus IQ Scan'){
            steps {
                script{
                    try {
                    def nexusPolicyEvaluation = nexusPolicyevaluation failBuildOnNetworkError: true,
                        iqApplication: selectedApplication("${iqAppID}"),
                        iqScanPatterns: [[scanPattern: "**/*.${packaging}"]],
                        iqStage: "${iqStage}",
                        jobCredentialsId: '',
                        callflow: [
                            enable: true,
        ...
        stage('Create tag'){
            steps {
                script {
                    ...
                    stableText: "Nexus Repository Tag: ${BUILD_TAG}",
        ...
        stage('Upload to Nexus Repository'){
            steps {
                script {
                    ...
                    mavenAssetList: [[classifier: '', extension: "${packaging}",
                    filePath: "${ARTEFACT_NAME}"]],
                    mavenCoordinate: [artifactId: "${artifactId}",
                    groupId: "${groupId}",
                    packaging: "${packaging}",
                    version: "${version}"]]],
                    tagName: "${BUILD_TAG}"
